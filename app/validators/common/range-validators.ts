...
export const validateFromSumRange = (fromSum?: number, toSum?: number): ValidatorMessageType =>
  validateNumberRange(fromSum, toSum) && `אנא הזן ערך קטן או שווה ל ${formatHebrewNumber(toSum)}`;

export const validateToSumRange = (fromSum?: number, toSum?: number): ValidatorMessageType =>
  validateNumberRange(fromSum, toSum) &&
  `אנא הזן ערך גדול או שווה ל ${formatHebrewNumber(fromSum)}`;
